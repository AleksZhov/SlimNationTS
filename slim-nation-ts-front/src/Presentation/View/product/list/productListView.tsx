    ...
    return (<div className="pt-[20px]">
        ...
        <ul className=" mt-6 flex gap-4 flex-wrap mx-5">
            {products.map((product) => {
                return <li className="w-[250px] rounded-xl bg-gradient-to-t from-indigo-500 to-purple-500 p-[20px] relative" key={product.id}>
                    <Button className="absolute top-1 right-2 border-2 border-black rounded-full px-[6px]" buttonLabel="X" type="button" onClick={async() => {
                        const { result } = await DeleteProductUseCase(product.id);
